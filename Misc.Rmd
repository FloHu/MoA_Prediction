...
```{r}
(load("./data/programmatic_output/my_fingerprint_mcl.RData"))
m <- readRDS("./data/programmatic_output/the_matrix_matrixcontainer_alldosg_2019.rds")
...
tSNE with same matrix:

```{r}
set.seed(5)
tsne_input <- m_matrix
m$drug_conc <- paste0(m$drugname_typaslab, "_", m$conc)

tsne <- Rtsne::Rtsne(X = tsne_input, dims = 3, perplexity = 10, max_iter = 2000)
tsne_data <- data.frame(tsne$Y)
colnames(tsne_data)[1:3] <- c("tSNE1", "tSNE2", "tSNE3")
tsne_data$drug_conc <- m$drug_conc
tsne_data <- left_join(tsne_data, m[, c("drug_conc", "process_broad")])

ggplot(tsne_data, aes(x = tSNE1, y = tSNE2, colour = process_broad)) + 
  geom_point(alpha = 0.75) + 
  scale_colour_manual("MoA", values = moa_cols) + 
  theme(legend.position = "None")

ggsave(filename = "./plots/tSNE_allfeats.pdf", width = 3.5, height = 3.5)
```

tSNE for fingerprint:
...
set.seed(5)
tsne_input <- m_matrix
tsne_input <- tsne_input[, my_fingerprint_mcl]
...
ggsave(filename = "./plots/tSNE_fingerprint.pdf", width = 3.5, height = 3.5)
...
With fingerprint features:

```{r}
(load("./data/programmatic_output/my_fingerprint_mcl.RData"))

h <- Heatmap(matrix = m[, my_fingerprint_mcl], 
  col = colorRamp2(breaks = c(-5, 5), colors = c(min_col, max_col)), 
  name = "S-score", 
...
pdf("./plots/Heatmap_fingerprint_euclidean.pdf", width = 10, 
  height = 20)
h
dev.off()
```
...
library(htmlTable)
cat(htmlTable(probdiffs_big), file = "./data/programmatic_output/probdiffs.html")
```
...
# overlap (or less?) I would expect between drugs of different MoA than within the same MoA
