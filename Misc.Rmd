...
```{r plot_flasthalk}
tmp <- readRDS("./data/programmatic_output/matrix_container_withextractions.rds")
...
matrix_container_with_extractions <- readRDS("./data/programmatic_output/matrix_container_withextractions.rds")
...
This concerns all dosages, lasso vs. random forests, focus only on top20pct
...
filter_container(matrix_container, dosgs = "all", feats = "top20pct", 
  models = "classif.randomForest", chemfeats = TRUE)

tmp <- filter_container(matrix_container_ext, dosgs = "all", feats = "top20pct", 
  models = c("classif.glmnet", "classif.randomForest"))

pdf("./plots/ROC_curves_plusminuschem.pdf", width = 15, height = 10)
plot_ROC_from_container(tmp)
dev.off()
```
...
```{r}
tmp <- filter_container(tmp, models = "classif.randomForest")
# turn into a function with a default set of columns? 
tmp_unnest <- 
  select(tmp, drug_dosages, feat_preselect, chemical_feats, fitted_model, 
    PredDataWithNSignif) %>%
  unnest() %>%
