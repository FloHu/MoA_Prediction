...
```{r}
plot_heatmap <- function(m  , plotFile = F ) {
...
a = plot_cat_lasso_coeffs(resObj = lasso_10pc_most, moa = "dna", main= "lasso_10pc_most", thres = 0.5, return_obj = T)
# Load matrix correpsonding to the model : dosage with most interaction
mat = matrix_container[20, ]$drug_feature_matrices[[1]]
mat = mat %>% select(drugname_typaslab, process_broad, names(a))
plot_heatmap(m = mat)
...
for( moa in c("cell_wall", "dna", "membrane_stress", "protein_synthesis")){
    ...
    for (i in colnames(matTmp)) {
        if(i != "process_broad"){
            res[[i]] = t.test(matTmp[matTmp$process_broad == moa, i], matTmp[matTmp$process_broad != moa, i])$p.value
        }
    }
