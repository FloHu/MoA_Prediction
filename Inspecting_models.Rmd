...
```{r}
plot_heatmap <- function(m  , plotFile = F, name_size = 0.3 ) {
    ...
    heatmap.2(cluster_matrix,
              trace = "none",
              breaks = c(seq(-10,10,length=50)), # use this i/o zlim
...
drugs_pred_prob_from_container(pred_data = matrix_container[155, ]$PredData[[1]], moa="cell_wall" )
...
drugs_pred_prob_from_container(pred_data = matrix_container[180, ]$PredData[[1]], moa="protein_synthesis" )
...
Potentialy TOLB, SPR, SlogP, ACRB and ACRA but also GLMS, ASMA, RFAE
...
```{r}
# Best model based on AUC : line 172
...
Clusters of membrane stress drugs seem driven by SURA, PAL, RFAF but also maybe with SlogP and IMP (medium values, look at the boxplot repartition, it's funny)
...
drugs_pred_prob_from_container(pred_data = matrix_container[165, ]$PredData[[1]], moa="dna" )
...
plotData = cbind(plotData, process_broad = mat$process_broad)
...
p <- plot_ly(plotData, x = ~PC1, y = ~PC2, z = ~PC3, color = ~process_broad, colors = c("#FF0000FF", "#80FF00FF", "#00FFFFFF","white", "white", "white", "#8000FFFF") ) %>%
  add_markers() %>%
...
input_data = input_data %>% select(process_broad, best_feat)
    
predictMoa = makeClassifTask(data = input_data, target = "process_broad")
    
lrn = makeLearner("classif.knn", par.vals = list(k = 1)) 
