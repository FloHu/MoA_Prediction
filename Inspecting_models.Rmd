...
p1 <- plot_ly(plotData, x = ~tSNE1, y = ~tSNE2, z = ~tSNE3, color = ~MoA, 
  ...
  layout(title = "Fingerprint tSNE map", scene = list(xaxis = list(title = 'tSNE1'), 
...
filter(mcl_imps, gene %in% top50_mcl) %>%
  mutate(imp_log2 = log2(importance), 
    gene = fct_reorder(gene, imp_log2, .desc = TRUE)) %>%
  ...
  #coord_flip() + 
  labs(title = "Top 50 features driving MoA prediction", 
    x = "Feature driving prediction", y = "Log2 feature importance") + 
  theme(text = element_text(size = 12), axis.text.x = element_text(size = 6, 
    angle = 45, hjust = 1))

ggsave(filename = "./plots/Fingerprint_mcl.pdf", width = 7, height = 5)
...
(msplit <- split(m, m$process_broad))

(msplit <- map(msplit, function(m) {
  rownames(m) <- paste0(m$drugname_typaslab, "_", m$conc, " (", 
  m$mic_curated, ")")
  
  m <- select(m, -drugname_typaslab, -conc, -process_broad, 
  -process_subgroup, -mic_curated) %>%
  select(my_fingerprint_mcl) %>% # cannot be mixed with previous select statement
  as.matrix()
...
for (mode in moas) {
  ...
  pdf(paste0("./plots/Fingerprint_mcl_heatmap", mode, ".pdf"), width = 9, 
    height = 7.8)
  print(h)
  dev.off()
}
