...
chemogeno_fingerpint = c("MRCB", "SLT", "YEJE", "YBAB",
                         "RECG", "XERC", "YDFI", "MOTA", "TPX", "YGFA", "RECN", "HUPA",
                         "RYFC", "YAIU", "GLXK", "YGAZ_YGAH", "YHFW", "YGAT", "DSRB", "KSGA",
                         "RFAF", "YJHQ", "YRAP", "YJJG", "YDCH", "PLDB", "YBGC")
...
fingerprint_mat = full_mat %>% select(drugname_typaslab, process_broad, chemogeno_fingerpint)


m = fingerprint_mat %>% select(chemogeno_fingerpint)
...
randomFeat = sample(colnames(full_mat)[-c(1,2,3)], size = length(chemogeno_fingerpint), replace = F)

knn_res = list()
# Don't test with 1 cause it might be the replicate with another dosage
for(feat in list(chemogeno_fingerpint, randomFeat)){
    for(nb_knn in c(3,5,7,9,11)){
        print(length(knn_res))
        name =  as.character(length(knn_res))
        knn_res[[name]] <- run_KNN(feat_subset = feat, knn_k = nb_knn, dataMat = full_mat)
    }
}
...
colnames(a) = "mmce"
a$moa = rep(c("cell_wall", "dna", "membrane_stress", "protein_synthesis"), times = 5)
a$knn = rep(c(3,5,7,9,11), each = 4)
a$model = rep(c("chemogeno_fingerpint", "randomFeat"), each = 20)


p1 = ggplot(data = a[1:20, ]) + geom_line(mapping = aes(x = knn, y = mmce, color = moa), lwd = 1.5) + theme_bw() + scale_color_manual(values = rainbow(4)) + ggtitle("chemogeno_fingerpint") + theme(legend.position="bottom")+ ylim(0,0.5)
p2 = ggplot(data = a[21:40, ]) + geom_line(mapping = aes(x = knn, y = mmce, color = moa), lwd = 1.5) + theme_bw() + scale_color_manual(values = rainbow(4)) + ggtitle("randomFeat") + theme(legend.position="bottom")+ ylim(0,0.5)
