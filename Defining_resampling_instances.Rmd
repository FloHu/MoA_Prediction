...
dataset_allDosage <- readRDS("./data/programmatic_output/the_matrix_matrixcontainer_alldosg.rds")
...
# instance_oneDrug <- Rep_Nest_CV_instance_newDrugs
...
Rep_Nest_CV_instance_allDosage <- 
  RepNCV_instance_map_drugname(instance_oneDrug = Rep_Nest_CV_instance_newDrugs, 
    dataset_oneDosage = dataset_newDrugs, dataset_allDosage = dataset_allDosage)
```
...
Run tests on the cv instances:

```{r testing_cvinsts_I}
run_cv_tests(Rep_Nest_CV_instance_newDrugs, dataset_newDrugs)
run_cv_tests(Rep_Nest_CV_instance_allDosage, dataset_allDosage)
```
...
run_cv_tests(cvinst_mostias, dataset_newDrugs)
run_cv_tests(cvinst_alldosg, dataset_allDosage)
...
data <- dataset_allDosage
```


Note that the cv instance currently used in the matrix_container for all dosages is not derived 
from the one for most interactions. See also the following chunk. But since tests passed there is 
...
cvtest_report_stratification(cvinst_alldosg$NCV_1, dataset_allDosage)$outer
cvtest_report_stratification(cvinst_mostias$NCV_1, dataset_newDrugs)$outer

# to be more precise:
tab <- cvtest_report_stratification(cvinst_mostias$NCV_1, dataset_newDrugs, FALSE)$outer
filter(tab, test.or.train == "test", MoA == "cell_wall", split == "a")

# whereas
tab2 <- cvtest_report_stratification(cvinst_alldosg$NCV_1, dataset_allDosage, FALSE)$outer
