...
plot_importance <- function(dfr) {
   ggplot(dfr, aes(x = gene, y = importance)) + 
      ...
      labs(title = "Feature importances for MoA dna", x = "Gene", y = "Importance (mean decrease in node impurity)")
}

feat_RF_dna_dfr <- make_importance_dfr(feat_RF_dna, cutoff = 0.05)
plot_importance(feat_RF_dna_dfr)
ggsave("./plots/present_feat_imp_dna.pdf", width = 7, height = 5)
...
feat_xgbt_dfr <- make_importance_dfr(feat_XGBT_dna)
plot_importance(feat_xgbt_dfr)
ggsave("./plots/present_feat_imp_xgbt.pdf", width = 7, height = 5)
...
### Most important features - visualise feature distributios
