...
p <- 
   ...
   theme_bw()
ggsave(plot = p, filename = "./plots/Prob_stabilities_by_feat_preselect.pdf")
...
Conclusion: Random forests is the most stable model, lasso the least stable one although the latter 
has a big difference between individual drugs. For boosted trees, stabilities don't seem to differ 
between different hyperparameter grids. 
...
p <- 
   ...
   geom_vline(xintercept = c(1.5, 2.5, 3.5), linetype = "dotted") + 
   facet_grid(fitted_model ~ feat_preselect) + 
   labs(x = "Mode of action modelled", y = "Standard deviation of probabilities", 
...
The difference is probably not significant for lasso. Again, there is no influence of the 
hyperparameter grid used for boosted trees. 
...
p <- 
   ggplot(foo, aes(x = process_broad, y = prob.moa_sd)) + 
   geom_boxplot(aes(colour = moa_modelled_is_truth), position = position_dodge(), outlier.shape = NA) + 
   geom_point(position = position_jitterdodge(jitter.width = 0.2), aes(colour = moa_modelled_is_truth), size = 1, alpha = 0.5) + 
   geom_vline(xintercept = c(1.5, 2.5, 3.5), linetype = "dotted") + 
   facet_grid(fitted_model ~ feat_preselect) + 
   labs(x = "Mode of action modelled", y = "Standard deviation of probabilities", 
...
except for lasso where it's vice versa. Interestingly, with all dosages, predictions are always more 
stable if MoA != MoA modelled. 
...
#### AUCs and AUC stabilities by model, feat_preselect and hyperparameter grid. 
...
p <- 
   ggplot(foo, aes(x = feat_preselect, y = auc)) + 
   geom_boxplot(aes(colour = moa_modelled), position = position_dodge(), outlier.shape = NA) + 
   geom_point(position = position_jitterdodge(jitter.width = 0.1), aes(colour = moa_modelled), size = 1, alpha = 0.5) + 
...
ggsave(plot = p, filename = "./plots/AUC_stabilities_alldosages.pdf", width = 16)
```
...
whereas top5pct is best for lasso. For lasso performance seems to drop with the higher number of 
features that are kept. 
* `dna` also has a decent performance which is again quite stable. For lasso again the performance 
seems to drop with the increasing number of features. But perhaps it also just reflects the 
...
p <- 
   ggplot(foo, aes(x = moa_modelled, y = auc, colour = drug_dosages)) + 
...
p
ggsave(plot = p, filename = "./plots/AUC_stabilities_alldosages_vs_mostinteractions.pdf", width = 16)
```

Conclusions: 
* RF: 
   + cell_wall and dna: aucs are very similar between all dosages and most_interactions. 
   + For membrane_stress, most_interactions always performs worse. 
...
p <- 
   ggplot(foo, aes(x = moa_modelled, y = auc, colour = chemical_feats)) + 
   geom_boxplot(position = position_dodge(), outlier.shape = NA) + 
   geom_point(position = position_jitterdodge(jitter.width = 0.1), size = 1) + 
   #stat_summary(fun.y = "median", geom = "point", colour = "red", size = 2) + 
   geom_vline(xintercept = c(1.5, 2.5, 3.5, 4.5), linetype = "dotted") + 
...
testrow <- 
   filter(matrix_container_ext, fitted_model == "classif.randomForest", feat_preselect == "keepall", 
          chemical_feats == FALSE, drug_dosages == "all")
testrow_pred <- testrow$PredData[[1]]
testrow_drugfeatmat <- testrow$drug_feature_matrices[[1]]
head(testrow_pred)
#subfr <- testrow_pred[testrow_pred$drugname_typaslab == "AMPICILLIN", ]

# to get this one to work: need to get `the_matrix` from first notebook after the do(select_mutant(.)) step
group_by(the_matrix, drugname_typaslab, conc) %>%
   summarise(ngenes = n(), nsignif = sum(significant))

# to get a correlation of the number of significant interactions and the prediction probabilities
# need to do a lookup from drug_feature_matrices in matrix_containter to the_matrix
head(the_matrix) # contains for each gene - drug - dosage combination whether it's significant or not - so we can count
head(testrow_drugfeatmat) # the matrix that was actually used for the model

# (1) gather the actually used drug-feature matrix
a <- testrow_drugfeatmat %>%
   select(-process_broad) %>%
   gather(3:ncol(.), key = "gene_synonym", value = "sscore")

# (2) join with the_matrix, group by drugname_typaslab and concentration to count number of significant interactions
b <- left_join(a, the_matrix) %>%
   group_by(drugname_typaslab, conc) %>%
   summarise(n_signif = sum(significant, na.rm = TRUE)) # because some drug-feature-conc combinations will be NA

# (3) feed information of b back into our prediction object 
c <- left_join(testrow_pred, b)
head(c)
c$moa_modelled_is_truth <- c$moa_modelled == c$process_broad

# we want to see medians, not everything
subfr2 <- group_by(c, drugname_typaslab, conc, moa_modelled, moa_modelled_is_truth, n_signif) %>% # will generate only the smallest groups
   summarise(median_prob.moa = median(prob.moa))

subfr2_amp <- subfr2[subfr2$drugname_typaslab == "AMPICILLIN", ]

# ! conc needs to be a factor for boxplot dodging to work
ggplot(subfr2_amp, aes(x = factor(conc), y = median_prob.moa, colour = moa_modelled)) + 
   geom_boxplot() + 
   geom_point(position = position_jitterdodge(jitter.width = 0.2)) #+ 
   #scale_x_discrete(labels = paste0(subfr2$conc, ":", subfr2$n_signif)) # not working

# global level:
ggplot(subfr2, aes(x = n_signif, y = median_prob.moa)) + 
   geom_point(shape = 1) + 
#   geom_line(aes(group = drugname_typaslab)) + 
   facet_grid(moa_modelled ~ moa_modelled_is_truth) + 
   theme_bw()

# to get an idea of the individual drugs
ggplot(subfr2[subfr2$moa_modelled == "cell_wall" & subfr2$moa_modelled_is_truth, ], 
...
ggplot(subfr2[subfr2$moa_modelled == "dna" & subfr2$moa_modelled_is_truth, ], 
       aes(x = n_signif, y = median_prob.moa, colour = drugname_typaslab)) + 
   geom_point(shape = 1) + 
   geom_line(aes(group = drugname_typaslab)) + 
   facet_grid(moa_modelled ~ moa_modelled_is_truth) + 
