...
# # just demonstrate what the loop does
# # test set performances
# get_perf_measures_mcl(resobj = res_new)
# get_pred_data_mcl(resobj = res_new)
# 
# # to be added: 
# # - routine for confusion matrix data
# # - MoA information, number of significant interactions, how many 
# #   concentrations the drug had etc.
# # - model stabilities (possibly outside of loop)
# 
# # hyperparameter tuning metrics: 
# # first just get the optimal hyperparameters + their performance (in the inner 
# # loop) - may get whole paths later on if desired 
# 
# get_opt_path(resobj = res_new)
# get_opt_pars(resobj = res_new)
...
select(mc_ext, fitted_model, drug_dosages, chemical_feats, perf_measures) %>%
  ...
  ggplot(aes(x = fitted_model, y = mmce_mean)) + 
    geom_boxplot(outlier.shape = NA) + 
    geom_point(position = position_jitter(width = 0.15, height = 0), shape = 1) + 
...
select(mc_ext, fitted_model, drug_dosages, chemical_feats, perf_measures) %>%
  ...
  group_by(fitted_model, drug_dosages, chemical_feats, cvrep) %>%
  summarise(mmce_mean = mean(mmce), kappa_mean = mean(kappa)) %>%
  ggplot(aes(x = fitted_model, y = kappa_mean)) + 
    geom_boxplot(outlier.shape = NA) + 
    geom_point(position = position_jitter(width = 0.15, height = 0), shape = 1) + 
    facet_grid(chemical_feats ~ drug_dosages)

# stabilities - how to deal with cv repeats? 
...
# comparison inner and outer performances
tmp <- 
  select(mc_ext, fitted_model, drug_dosages, chemical_feats, perf_measures, 
  opt_pars) %>%
  unnest()
...
ggplot(tmp, aes(x = mmce.cv.inner, y = mmce)) + 
  geom_point() + 
  coord_fixed() + 
  facet_grid(chemical_feats ~ drug_dosages)

ggplot(tmp, aes(x = mmce - mmce.cv.inner)) + 
  geom_histogram() + 
  facet_grid(chemical_feats ~ drug_dosages)
...
tmp2 <- filter(tmp, !chemical_feats, drug_dosages == "one")

ggplot(tmp2, aes(x = factor(opt_pars_value))) + 
  geom_bar() + 
  facet_grid(drug_dosages ~ opt_pars_name, scales = "free_x")
...
Prediction heatmap, prediction lines, ROC curves, confusion matrix.
...
tmp <- mc_ext$pred_data[[5]]
melt_pred_data_mcl(tmp)
plot_mcl_probs_heatmap(melt_pred_data_mcl(tmp), mics = mics)

# profiles to see probabilities better
plot_mcl_probs_lines(melted_pred_data = melt_pred_data_mcl(tmp), 
  labels = moas, colours = moa_cols)

pdf(width = 20, height = 20, file = "./plots/tmp.pdf")
plot_mcl_probs_lines(melted_pred_data = melt_pred_data_mcl(tmp), 
  labels = moas, colours = moa_cols)
dev.off()

# ROC curves
plot_roc_mcl(resampled_multiclass = tmp, positives = moas)
```

Calibration plot.

```{r}
plot_prob_calib(resample_result = tmp, title = "muuuh!")
```
...
```{r}
if (file.exists("./data/programmatic_output/matrix_container_ext_2019.rds")) {
  matrix_container <- readRDS("./data/programmatic_output/matrix_container_ext_2019.rds")
} else {
...
the_matrix_complete_strainsgenesremoved <- 
  readRDS("./data/programmatic_output/kept_datasets_long_featsndrugs_removed.rds")$nichols_2011

# (foo <- readRDS("./data/programmatic_output/kept_datasets_long_featsndrugs_removed.rds"))

# loop to read in results files
indir <- "./run_results_from_server/matrix_container_result_2019"
...
#     pattern = "xgboost|rf|lasso")
   available_files <- list.files(indir, pattern = "xgboost|rf|lasso")
  ...
  my_match <- available_files[available_files == targetfile]
   ...
   resultsobj <- readRDS(file.path(indir, my_match))
...
matrix_container$feat_preselect <- factor(matrix_container$feat_preselect, 
  levels =  c("top5pct", "top10pct", "top15pct", "top20pct", "top25pct", 
    "top30pct", "top40pct", "top50pct", "keepall"))

saveRDS(object = matrix_container, file = "./data/programmatic_output/matrix_container_ext_2019.rds")
```
...
model_cols <- c("Boosting" = "#7570b3", "Lasso" = "#d95f02", 
  "Random forests" = "#1b9e77")
moa_cols <- c("Cell Wall" = "#1b9e77", "DNA" = "#d95f02", 
  "Membrane Stress" = "#7570b3", "Protein Synthesis" = "#e7298a")
...
# updated. 
# (matrix_container <- readRDS("./data/programmatic_output/matrix_container_2019.rds"))
...
```{r}
(matrix_container <- matrix_container[!is.na(matrix_container$pred_data), ])
```
...
# for GB seminar:
# ggplot(foo, aes(x = feat_preselect, y = prob.moa_sd2, colour = fitted_model, 
#   group = fitted_model)) + 
#   stat_summary(geom = "line", fun.y = median) + 
#   stat_summary(geom = "point", fun.y = median) + 
#   stat_summary(geom = "errorbar", fun.y = median, width = 0.2, alpha = 0.4, 
#     fun.ymin = function(x) {fivenum(x)[2]}, 
#     fun.ymax = function(x) {fivenum(x)[4]}) + 
#   facet_wrap( ~ process_broad) + 
#   labs(x = "Subset of features kept", y = "Model instability [median s.d. of predictions +- IQR]", 
#     title = "Model instabilities: lasso vs. random forests") + # may wanna add a subtitle ... 
#   theme_bw() + 
#   theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10), 
#     text = element_text(size = 14), panel.grid = element_blank()) + 
#   scale_colour_manual("Model", values = model_cols[2:3], 
#     labels = names(model_cols)[2:3])
# 
# ggsave("./plots/Stability_comp_RF_Lasso_alldosg_nochem.pdf", width = 8, 
#   height = 6)
...
ggplot(filter(foo, moa_modelled == "Membrane Stress"), aes(x = feat_preselect, y = auc, colour = drug_dosages)) + 
  ...
  labs(x = "Subset of features kept", y = "Model performance [AUC]", 
    title = "Impact of dosage and feature selection\non model performance") + # may wanna add a subtitle ... 
  theme_bw() + 
...
plot_auc_vs_moa(foo, xaxis = "feat_preselect", yaxis = "prob.moa_sd2", 
  colourvar = "drug_dosages", groupvar = "drug_dosages", 
  facet_expr = quote(facet_grid(fitted_model ~ process_broad)), 
  xlab = "Feature subset", ylab = "Model instability", 
  title = "Model instabilities: impact of dosage selection", 
...
There may be reasons though to work without chemical features and fewer dosages 
(see below). This would impact our performances as follows. I'm taking only 
top20pct and keepall for simplicity as they are the best feat_preselect cases 
depending on the MoA. 

```{r}
tmp <- filter_container(matrix_container, models = c("classif.randomForest", 
  "classif.glmnet"), dosgs = "all", feats = c("top20pct", "keepall")) %>%
  select(fitted_model, drug_dosages, feat_preselect, chemical_feats, 
    perf_measures) %>%
  unnest()

plot_auc_vs_moa(tmp, xaxis = "moa_modelled", yaxis = "auc", 
  colourvar = "chemical_feats", 
  facet_expr = quote(facet_grid(feat_preselect ~ fitted_model)), 
  additional_info = "")
```

So one may be able to compensate for the loss of chemical features by using 
lasso. Here are the partial AUCs:

```{r}
plot_auc_vs_moa(tmp, xaxis = "moa_modelled", yaxis = "part_auc_01", 
  colourvar = "chemical_feats", 
  facet_expr = quote(facet_grid(feat_preselect ~ fitted_model)), 
  additional_info = "")
```
