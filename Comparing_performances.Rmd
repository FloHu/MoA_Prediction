...
ggplot(foo, aes(x = feat_preselect, y = auc, colour = drug_dosages)) + 
  ...
  labs(x = "Subset of features kept", y = "Model performance [AUC]", 
    title = "Model performances: impact of dosage and feature selection") + # may wanna add a subtitle ... 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10), 
    text = element_text(size = 14), panel.grid = element_blank()) + 
...
ggplot(tmp, aes(x = n_signif, y = median_prob.moa, colour = moa_modelled)) + 
  geom_point() + 
  theme_bw() + 
  facet_wrap( ~ moa_modelled) + 
...
ggsave("./plots/Probs_vs_nsignif.pdf", width = 6, height = 4.5)
