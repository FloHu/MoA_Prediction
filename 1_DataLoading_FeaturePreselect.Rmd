...
# the warning messages come from the 8 drugs where there is just one dosage
selector_2dosages <- 
   the_matrix_signifs %>%
   group_by(drugname_typaslab) %>%
   arrange(conc, .by_group = TRUE) %>%
...
And also get rid of the "other" MoA (different from cell_wall, dna, membrane_stress, 
protein_synthesis), reducing the number of drugs from 78 to 67 (5x pmf, 5x oxidative_stress, 1x 
protein_qc). 

```{r}
# continuing from line 213
the_matrix_mostias_nochemfeats_2dosages <- 
   left_join(the_matrix_mostias_nochemfeats_2dosages, my_labels_knowndrugs)
...
saveRDS(object = the_matrix_mostias_nochemfeats_2dosages, file = "data/programmatic_output/matrix_2mostia_mainMoA.rds")
```

The saved matrix can be used (loaded) to run a resampling instance creation with the appropriate 
notebook. Once generated, the resampling object need to be loaded.
...
```{r}
mat_corr07 <-  
   corr_feat_selection(data = 
                          the_matrix_mostias_nochemfeats_2dosages %>% 
                          select(-drugname_typaslab, -conc, -process_broad), 
                       thres = 0.7)
mat_corr07 <-  
   the_matrix_mostias_nochemfeats_2dosages %>% 
   select(drugname_typaslab, conc, process_broad, colnames(mat_corr07))
...
mat_corr06 <-  
   the_matrix_mostias_nochemfeats_2dosages %>% 
   select(drugname_typaslab, conc, process_broad, colnames(mat_corr06))
...
mat_corrBonf01 <- 
   the_matrix_mostias_nochemfeats_2dosages %>% 
   select(drugname_typaslab, conc, process_broad, colnames(mat_corrBonf01))
...
mat_corrBonf05 <- 
   the_matrix_mostias_nochemfeats_2dosages %>% 
   select(drugname_typaslab, conc, process_broad, colnames(mat_corrBonf05))
```

Then based on protein complexes from the EcoCyc database. The average signal for a complex is 
computed and defined as a new feature.
...
mat_cmplx = 
   mat_cmplx %>% 
   mutate("drugname_typaslab" = the_matrix_mostias_nochemfeats_2dosages$drugname_typaslab, 
...
Extreme feature selection : First take the Protein complexes matrix, and apply the correlation based 
selection with 0.6 or 0.7 cutoff. Then look at the variance across all drugs. 
By boxploting these variances, one can extract the outlier individuals (based on some distribution 
and IQR, outlier are individuals with values above (median + 1.5 * IQR)). It is similar in a way to 
the top 10% or top 15% of individuals with the highest variance but it make more sense to define the 
cutoff that way. It is in this case a kind of data-dependent cutoff.
...
```{r}
instance_2mostia_mainMoA = readRDS(file = "data/programmatic_output/RNCV_instance_2mostia_mainMoa.rds")
...
saveRDS(object = matrix_container_new, file = "data/programmatic_output/matrix_container_new.rds")
```
