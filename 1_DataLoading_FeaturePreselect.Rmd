...
author: "Florian Huber, L??onard Dubois"
date: "`r Sys.Date()`"
output:
  html_document:
    fig_caption: yes
...
Here, one has to choose to build the Matrix with MoA to predictm or all MoA except "unknown"
...
```{r eval = F}
save("the_matrix", "labels", file = "data/the_matrix.RData")
```
...
Use of the `rfcv` function of `randomForest` package to assess error related to number of features used
...
for (moa in c("dna", "protein_synthesis", "cell_wall", "membrane_stress")) {
    ...
    res = rfcv(trainx = select(dt, -process_broad), trainy = dt$process_broad, cv.fold = 8, scale = "", step = -1, ntree = 250)

    plot(res$error.cv, type = "l", xaxt = "n", ylab = "CV error", xlab = "Number of features used", main = moa)
    axis(1, res$n.var, at = seq(1,length(res$n.var)))  
}
