...
This section may move to `2_Building_Models.Rmd` later on. 


### Taking genes with highest variance

Not sure if needed, but was part of initial prediction runs last year (2017). We take all chemical 
features and the 200 genes with the highest variance. Perhaps not surprisingly, it turned out that 
this yielded a very similar set to taking the 200 genes contributing most to the most important 
principal components. 

```{r highest_var}
genes_only <- select(the_matrix, -drugname_typaslab, -(SlogP:data_effective_rotor_count))
high_to_lowvar <- sort(apply(genes_only, 2, var), decreasing = T)

# these are the 200 genes with the highest variance
selected_genes <- names(high_to_lowvar[1:200])
...
Again, was part of the first few prediction attempts last year. Idea: do hierarchical clustering of 
all genes based on the correlations of their s-scores, then make 200 clusters and select the 
...
feature_container <- list() # will contain the respective genes 
