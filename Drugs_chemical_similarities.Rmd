...
print(plot_dend(drugs_fingerprint = drugs_RDkit_fingerprint, main = "Hclust of binary distance - RDKit fingerprint"))
dev.off()
=======
...
# plot_dend(drugs_fingerprint = drugs_RDkit_fingerprint, main = "Hclust of binary distance - RDKit fingerprint")
# dev.off()
>>>>>>> 0edf0c4563b4181c4378777fe32c17cd3e85b89b
```

```{r}
plot_dend(drugs_fingerprint = drugs_MACCS_fingerprint, main = "Hclust of binary distance - MACCS fingerprint")

pdf("./plots/fingerprint_MACCS_hclust.pdf", width = 10, height = 10)
print(plot_dend(drugs_fingerprint = drugs_MACCS_fingerprint, main = "Hclust of binary distance - MACCS fingerprint"))
dev.off()
```
...
pdf("./plots/fingerprint_morgan_hclust.pdf", width = 10, height = 10)
print(plot_dend(drugs_fingerprint = drugs_morgan_fingerprint, main = "Hclust of binary distance - Morgan fingerprint"))
dev.off()
```
...
# CecropinB is an outlier strongly influencing final plot
...
pdf("./plots/chemgen_dist_hclust.pdf", width = 10, height = 10)
par(mar = c(3,2,2,8)) 
dend_chemgen %>%
    set("labels_cex", 0.7) %>%
    set("branches_lwd", 1.5) %>%
    plot(horiz = T, cex = 0.5, main = "Hclust of euclidian distance - Chemogenomic profile")
dev.off()
```


Use (1 - correlation) as a distance
...
corr_mat = cor(t(chemgen_mat))

dist_chemgen = as.dist(1-corr_mat)
...
labels_colors(dend_chemgen) <- color_moa[the_matrix_allDrugs[order.dendrogram(dend_chemgen), "process_broad"]]
pdf("./plots/chemgen_corr_hclust.pdf", width = 10, height = 10)
par(mar = c(3,2,2,8)) 
dend_chemgen %>%
    set("labels_cex", 0.7) %>%
    set("branches_lwd", 1.5) %>%
    plot(horiz = T, cex = 0.5, main = "Hclust of (1-correlation) distance - Chemogenomic profile")
dev.off()
```
