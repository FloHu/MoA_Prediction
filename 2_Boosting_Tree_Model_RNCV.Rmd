...
bt_hyp_param = makeParamSet(
    ...
    makeDiscreteParam("max_depth", values = c(3)), #depth 1 = stump, 2 leaves for 4 classes, makes no sense 
    makeDiscreteParam("eta", values = c(0.1,0.3)), #Shrinkage to prevent overfitting
    makeDiscreteParam("lambda", values = c(0.1,0.5)) # regularization to prevent overfitting
)
bt_tuning = makeTuneControlGrid()
