...
fit_model_container_row <- function(row) {
  ...
  foreach(cvrep = icount(n_rep)) %do% {
    ...
    foreach(n = icount(length(train_test_pairs))) %do% {
      ...
      my_predict.type <- if (row_params$fitted_model == "classif.knn") {
        "response"
      } else {
        "prob"
      }
      lrn_train_outer <- makeLearner(cl = row_params$fitted_model, predict.type = my_predict.type)
...
perform_tuning <- function(.learner, .data, .task, .nfolds, .paramset, .ctrl) {
  ...
  opt_path <- generateHyperParsEffectData(tuning_result, partial.dep = TRUE) # changed 2019-04-19
