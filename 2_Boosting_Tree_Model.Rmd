...
This part is the general setup, whatever model we are using.
...
Boosting tree (BT) work in a similar way to Random Forest but trees are build sequentially. Each tree is a *weak* learner whose aim is to correct errors from the previous tree. In BT, trees present high bias and low variance and are sometimes as small as decision stump (one root and two leaves). By aggregating these weak learner, one reduces bias.
...
The values of arguments `mfinal` and `maxdepth` are not really important for they will be tuned in the inner loop of the cross validation
...
bt_expr = quote(res <- resample(learner = bt_learn, task = predictMoa, resampling = outer, extract = getFeatureImportance, models = T, measures = list(mmce, acc, kappa)))
