...
if (!file.exists(my_file)) {
  ...
  trained_model_loc <- mlr::train(learner = tuned_lrn, task = tsk)
...
Local importance scores: we can see that in the range of importance scores 
defining fingerprint features there is no difference between importance and 
local importance scores. 

```{r}
# remember: getFeatureImportance(learner) returns a 1-row data frame
l <- map(list(trained_model_prox, trained_model_loc), ~ t(getFeatureImportance(.x)$res)) %>% 
