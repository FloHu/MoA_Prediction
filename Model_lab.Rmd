...
# define categories ending up as NA, which we won't attempt to predict
moa_tab <- read_delim(outpath, delim = ",", na = c("-", "unknown", "pmf"))
moa_tab <- moa_tab[1:89, ]
table(moa_tab$`moa_broad_II (after 09/05/2018)`)
# remove NA drugs - either excluded or unknown
moa_tab <- moa_tab[!is.na(moa_tab$moa_subgroup), ]
table(moa_tab$moa_subgroup)
```

Run LOOCV with the subgroups (subject to change).
