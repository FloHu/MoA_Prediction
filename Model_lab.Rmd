...
(tmp <- left_join(feat_imps_multi, 
  feat_imps_1vsrest[, c("moa", "gene", "median_importance")], 
  by = "gene", suffix = c("_multi", "_1vsall")))

ggplot(tmp, aes(x = log2(median_importance_1vsall), 
  y = log2(median_importance_multi))) + 
  geom_point() + 
  facet_wrap( ~ moa) + 
...
plot_mcl_probs_heatmap(melted_pred_data = multicl_pred_melt, mics = mics) %>% 
  ggsave(filename = "./plots/Probability_heatmap_multiclassLOO.pdf", 
    width = 20, height = 20)
```
...
plot_mcl_probs_heatmap(melted_pred_data = multicl_2highest_melt, mics = mics) %>% 
  ggsave(filename = "./plots/Prediction_heatmap_LOO_2highest_heatmap.pdf", 
  width = 20, height = 20)
```
