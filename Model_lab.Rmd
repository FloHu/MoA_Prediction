...
Question is also: which correlation metric do we use? 

```{r}
# I'd suggest:
# (i) Correlation matrix 
# (ii) PCA
# (iii) Distributions of correlation
# (iv) Do i-iii by group (see above)
# (iii) Estimate threshold based on protein complexes

# ignore chemical features
...
# gtools::combinations() doesn't work because option expressions is too low
# when increasing it, it still doesn't work because of some C stack issue
# - the function is probably written in a recursive manner
# so let's reimplement it with a loop
from <- 1
to <- ncol(feats)
choose(to, 2)
(my_combs <- matrix(vector("numeric"), nrow = choose(to, 2), ncol = 2))

{
  counter <- 1
  for (i in seq_len(to-1)) {
    for (j in (i+1):to) {
      my_combs[counter, ] <- c(i, j)
      counter <- counter + 1
    }
  }
}

stopifnot(nrow(my_combs) == choose(to, 2) && (!any(is.na(my_combs))))

(my_rows <- my_combs[, 1])
(my_cols <- my_combs[, 2])

corpairs <- tibble(
  drugA = rownames(feats_cor)[my_rows], 
  drugB = colnames(feats_cor)[my_cols], 
  spear_cor = feats_cor[my_combs])
corpairs

ggplot(corpairs, aes(x = abs(spear_cor))) + 
...
corpairs$in_same_cmplx <- 
  map2_lgl(corpairs$drugA, corpairs$drugB, function(.drugA, .drugB) {
    drugpair <- c(.drugA, .drugB)
    if (!all(c(.drugA, .drugB) %in% genes_cmplx_v)) {
      FALSE
      } else {
...
ggplot(corpairs, aes(x = abs(spear_cor))) + 
  geom_histogram(bins = 45, aes(fill = in_same_cmplx)) + 
  #facet_wrap( ~ in_same_cmplx, scales = "free") + 
