...
Anticorrelation is exceedingly rare, only 16 pairs have a Spearman correlation of less than -0.6. 
...
Now delete (instead of merging) correlated predictors. Follow procedure from Kuhn et al, p47. 
...
2. Collinear features were removed
...
my_filename <- "./data/programmatic_output/resampled_multiclass_try1.rds"
if (file.exists(my_filename)) {
  resampled_multiclass_try1 <- readRDS(my_filename)
} else {
  resampled_multiclass_try1 <- resample(learner = rf_multi.learner, task = rf_multi.task,
    measures = mmce, resampling = rep_rin, models = TRUE, keep.pred = TRUE)
  # add drugname and concentration information to $pred$data slot
  resampled_multiclass_try1$pred$data <-
    bind_cols(my_data_mcl_preproc[resampled_multiclass_try1$pred$data$id, c(1, 2)],
      resampled_multiclass_try1$pred$data) %>%
    arrange(drugname_typaslab, conc, iter)
  saveRDS(resampled_multiclass_try1, file = my_filename)
}
```
...
```{r}
resampled_multiclass$aggr
resampled_multiclass_try1$aggr

compare_multiclass_models(resampled_multiclass, resampled_multiclass_try1, 
  "./plots/compare_multiclass_try1.pdf")
```
...
```{r}
compare_multiclass_models(resampled_multiclass_try1, resampled_multiclass_try2, 
...
suppressMessages(ggsave(filename = "./plots/Multiclass_probabilities_try2.pdf", plot = p, width = 30, 
  height = 20))
```
