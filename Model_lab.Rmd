...
Make default version of fitting function.
...
my_varname <- "resampled_multiclass"
my_filename <- file.path(outdir, paste0(my_varname, ".rds"))

if (file.exists(my_filename)) {
  assign(my_varname, readRDS(my_filename))
} else {
  assign(my_varname, fit_multiclass_rf_def(my_data_mcl))
  saveRDS(get(my_varname), my_filename)
}
```
...
```{r}
(multicl_pred_melt <- melt_pred_data(resampled_multiclass, 
...
```{r}
plot_mcl_probs_lines(multicl_pred_melt, labels = moas, colours = moa_cols)
ggsave(filename = "./plots/Multiclass_probabilities.pdf", width = 30, 
  height = 20)
```
...
ggsave(filename = "./plots/Multiclass_probabilities_micinfo.pdf", 
...
For predicting MoA subgroups we have to switch to LOOCV. 

Get default fitting function:
...
my_varname <- "resampled_mcl_loo"
my_filename <- file.path(outdir, paste0(my_varname, ".rds"))

if (file.exists(my_filename)) {
  assign(my_varname, readRDS(my_filename))
} else {
  assign(my_varname, fit_multiclass_rf_loo_def(mcl_loo))
  saveRDS(get(my_varname), my_filename)
}
```
...
```{r}
my_varname <- "resampled_mcl_2019"
my_filename <- file.path(outdir, paste0(my_varname, ".rds"))

if (file.exists(my_filename)) {
  assign(my_varname, readRDS(my_filename))
} else {
  assign(my_varname, fit_multiclass_rf_loo_def(my_data_mcl_2019))
  saveRDS(get(my_varname), my_filename)
}

resampled_mcl_2019 <- fit_multiclass_rf_loo_def(my_data_mcl_2019)
...
names(moa_tab) <- c("drugname_typaslab", "process_broad", "moa_subgroup")
moa_tab
...
unique(tm$drugname_typaslab)[!unique(tm$drugname_typaslab) %in% 
    (my_data_mcl_2019$drugname_typaslab)]
...
```{r}
my_varname <- "resampled_mcl_loo_subgroups"
my_filename <- file.path(outdir, paste0(my_varname, ".rds"))
...
if (file.exists(my_filename)) {
  ...
  assign(my_varname, 
    fit_multiclass_rf_loo(data = tm, targetvar = "moa_subgroup", 
      blockingvar = "drugname_typaslab", 
      paramlist = list(ntree = my_ntree, mtry = my_mtry), 
   cols_to_exclude = c("conc", "drugname_typaslab"), glue_back = TRUE))
  saveRDS(get(my_varname), my_filename)
}

plot_wide_confmat(get_wide_confmat(resampled_mcl_loo_subgroups), 
  title = "Multiclass RF, subgroups")
ggsave("./plots/subgroup_prediction.pdf")
```

# Predicting same drugs in Lucia's data set.
...
newsize <- filter(newsize, drugname_typaslab %in% 
    unique(my_data_mcl_2019$drugname_typaslab))
unique(newsize$drugname_typaslab)

newsize$sscore[is.na(newsize$sscore)] <- 0
newsize <- select(newsize, gene_synonym, drugname_typaslab, conc, sscore) %>%
  spread(key = gene_synonym, value = sscore)

newsize <- merge_features(newsize, clusters, mean)

newsize <- newsize[, colnames(newsize) %in% colnames(my_data_mcl_2019)]
newsize

newsize <- left_join(newsize, 
  my_subgroups_drugs[, c("drugname_typaslab", "process_broad")]) %>%
  select(drugname_typaslab, conc, process_broad, everything())
```

Retrain our multiclass 2019 model. 
...
my_rle <- rle(my_data_mcl_2019$drugname_typaslab)
my_blocks <- factor(rep(seq_along(my_rle$lengths), my_rle$lengths))

mcl_2019_task <- makeClassifTask(data = my_data_mcl_2019[, -c(1, 2)], 
  target = "process_broad", blocking = my_blocks)
mcl_2019_learner <- makeLearner("classif.randomForest", predict.type = "prob", 
  par.vals = list(ntree = my_ntree, mtry = my_mtry))
mcl_2019_model <- train(learner = mcl_2019_learner, task = mcl_2019_task)

getLearnerModel(mcl_2019_model)
feat_imps <- sort(unlist(getFeatureImportance(mcl_2019_model)$res), 
  decreasing = TRUE)
head(feat_imps, n = 20)

mcl_2019_pred <- predict(mcl_2019_model, newdata = newsize)
performance(mcl_2019_pred, measures = mmce)

mcl_2019_pred$data <- as_tibble(cbind(newsize[, 1:2], as.data.frame(mcl_2019_pred)))
View(mcl_2019_pred$data)
```


Comparing old with new predictions - but first make new way of displaying our 
results:

```{r}
# first come up with new way of displaying results:
resampled_mcl_2019_melt <- melt_pred_data(resampled_mcl_2019, "multiclass")
p <- plot_mcl_probs_heatmap(melted_pred_data = resampled_mcl_2019_melt, mics = mics)

ggsave(filename = "./plots/Prediction_heatmap_mcl_2019.pdf", plot = p, 
  width = 20, height = 20)
```

Heatmap of new predictions:

```{r}
class(resampled_mcl_2019)
class(resampled_mcl_2019$pred)
class(mcl_2019_pred)

head(resampled_mcl_2019$pred$data)
head(mcl_2019_pred$data)

mcl_2019_pred_melt <- melt_pred_data(mcl_2019_pred, "multiclass")
p <- plot_mcl_probs_heatmap(mcl_2019_pred_melt, mics = mics)
