...
So instead we write a function that defines one class as positive and the others 
as negative.
...
fpr. However, when the threshold approaches 0 there are quite a few observations 
that never "get the chance" to be put into the positive class. Why? Because 
their probability for the positive class (`cell_wall` in this case) is lower 
than the 2nd highest probability so they are just all classified as negative 
cases. Therefore, neither the TPR nor the FPR can ever go up again. Conclusion: 
`highest_vs_2nd` is not a sensible metric. 
...
# for blocking
my_rle <- rle(my_data_mcl_2high$drugname_typaslab)
my_blocks <- factor(rep(seq_along(my_rle$lengths), my_rle$lengths))
...
Comparison with previous multi-class model. 
...
* dna: split into:
  * folate: sulfa drugs and trimethoprim
  * dna: all others
* membrane_stress: split into: 
  * detergents: triton, sds, benzalkonium, bile, (deoxy/tauro-)cholate
  * fatty_acid: cerulenin, thiolactomycin, triclosan
* new groups: 
  * ox_stress: paraquat, peroxide, pms, pyocyanin
* not sure/not included/removed:
  * pmf: ??? b/c only 2 drugs (CCCP, nigericin) after removing dibucaine, procaine, verapamil
  * bacitracin (only gram+), CHIR090 (?), EGCG (?), INDOLICIDIN (?), Polymyxin B (?), 
  vancomycin (only gram+), cecropin B (?)
...
```{r}
# let's try and access the MoA table using the googlesheets package
# https://googledrive.tidyverse.org/
library(googledrive)
(gfiles <- drive_find(pattern = "drug_moa"))
outpath <- "./data/programmatic_output/drug_moa_gdrive.csv"
drive_download(file = gfiles[1, ], path = outpath, overwrite = TRUE)
moa_tab <- read_delim(outpath, delim = ",")
moa_tab <- moa_tab[1:89, ]
table(moa_tab$`moa_broad_II (after 09/05/2018)`)
