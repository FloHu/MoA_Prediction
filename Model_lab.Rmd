...
stopifnot(nrow(my_data_multiclass) == nrow(feats))
...
my_data_mcl_2high_train <- select(my_data_mcl_2high, -one_of(c("conc", "drugname_typaslab")))
rf_multi.task <- makeClassifTask(data = my_data_mcl_2high_train, target = "process_broad")
rf_multi.learner <- makeLearner("classif.randomForest", predict.type = "prob",
  par.vals = list(ntree = my_ntree, mtry = my_mtry))
```
...
Will remove this and put it into a function when all the issues with the 1-vs-rest models have been 
sorted out as well.
...
(ref_fracs <- table(my_data_mcl_2high$process_broad) / length(my_data_mcl_2high$process_broad))
cv_instances <- list()
counter <- 0
max_deviance <- 0.05
...
while (length(cv_instances) < 10) {
   ...
   for (split in c(1:8)) {
      train_fracs <- table(my_data_mcl_2high$process_broad[rin$train.inds[[split]]]) / length(rin$train.inds[[split]])
      # we don't want any "empty" classes in the test set:
      if (length(table(my_data_mcl_2high$process_broad[rin$test.inds[[split]]])) != length(moas)) {
         break
      }
...
if (file.exists(my_filename)) {
  ...
  resampled_multiclass_2high <- resample(learner = rf_multi.learner, task = rf_multi.task,
    measures = mmce, resampling = rep_rin_try2, models = TRUE, keep.pred = TRUE)
  # add drugname and concentration information to $pred$data slot
  resampled_multiclass_2high$pred$data <-
    bind_cols(my_data_mcl_2high[resampled_multiclass_2high$pred$data$id, c(1, 2)],
      resampled_multiclass_2high$pred$data) %>%
    arrange(drugname_typaslab, conc, iter)
