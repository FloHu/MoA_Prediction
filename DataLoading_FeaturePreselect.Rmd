...
Therefore, for Nichols and Newsize:
  ...
  * Finally, we also want to remove mutants that have no significant 
  interaction with any drug as they are likely to not yield any information. 
...
unique(the_matrix$drugname_typaslab)[!unique(the_matrix$drugname_typaslab) %in% 
    mics$drugname_typaslab]

dosg_check <- left_join(select(the_matrix, drugname_typaslab, conc), mics) %>% 
  mutate(frac_of_mic = conc / mic_curated) %>% 
  select(drugname_typaslab, mic_curated, conc, frac_of_mic, resistant)

group_by(dosg_check, drugname_typaslab) %>%
  filter(all(frac_of_mic < 0.05)) %>% 
  View()
```
