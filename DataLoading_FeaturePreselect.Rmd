...
my_distfun <- function(x) {as.dist(1 - x)}

# in the heatmap function below we want to split the dendrogram into nclust 
# clusters
# problem: the ordering of the clusters will be messed up as it follows the 
# levels of the output of cutree by default 
# however, we want it in the same order as the dendrogram: 

h <- hclust(feats_cor_high_d)
# now this is the order as in the dendrogram:
(dend_order <- rev(h$labels[order.dendrogram(as.dendrogram(h))]))
# dend_order %<>% factor(levels = dend_order)

nclust <- 23
clusters <- cutree(hclust(feats_cor_high_d), k = nclust)
my_split <- clusters
# usually clusters in Heatmap() would order stuff sorted by levels 1, 2, 3, ... 
# but if we subset first with dend_order we can put the levels in the right 
# order
my_split %<>% factor(levels = clusters[dend_order][!duplicated(clusters[dend_order])])

min_col <- plasma(2)[1]
max_col <- plasma(2)[2]

# almost right!
Heatmap(feats_cor_high, 
...
# this! :)
my_split2 <- my_split[order(my_split, rev(names(my_split)))]
...
Heatmap(feats_cor_high, 
  ...
  column_order = rev(dend_order), 
  # row_order = rev(dend_order), 
  split = my_split)
...
the_matrix_signifs <- filter(the_matrix_signifs, drugname_typaslab %in% 
    unique(drugs_subset$drugname_typaslab))
saveRDS(the_matrix_signifs, file = file.path(outdir, "the_matrix_signifs_keepall_wide.rds"))
```
