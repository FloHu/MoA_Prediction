...
drugs_kept_for_container
saveRDS(drugs_kept_for_container, file = file.path(outdir, "drugs_kept_for_container"))

drugs_rmvd_from_container <- 
  unique(the_matrix$drugname_typaslab[!the_matrix$drugname_typaslab %in% drugs_subset$drugname_typaslab])
drugs_rmvd_from_container
saveRDS(drugs_rmvd_from_container, file = file.path(outdir, "drugs_rmvd_from_container"))
...
saveRDS(dosg_tab_top3, file = file.path(outdir, "dosg_tab_top3"))
```
...
```{r}
mc <- bind_rows(rep(list(mc), 4))

mc$fitted_model <- 
  rep(c("classif.randomForest", "classif.glmnet", "classif.xgboost", 
    "classif.qda"), each = 4)
...
xgboost_grid <- makeParamSet(
  ...
  makeDiscreteParam("max_depth", values = c(3, 5, 6, 8)), 
  makeDiscreteParam("fw.perc", values = c(0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 
    0.5, 1))
)

# just out of curiosity, probably won't use because p >> n
qda_grid <- makeParamSet(
  makeDiscreteParam("fw.perc", values = c(0.005, 0.01, 0.015))
)
...
mc$hyperparam_grid <- 
  ifelse(mc$fitted_model == "classif.randomForest", 
    ...
    ifelse(mc$fitted_model == "classif.glmnet", 
      list(lasso_grid), 
      ifelse(mc$fitted_model == "classif.xgboost", 
        list(xgboost_grid), 
        list(qda_grid))))
