...
Number and fraction of observations affected: less than 2‰. 
...
Looks like 21 clusters is a good number to capture similar groups. 
...
Now merge the features by calculating the centroids for `the_matrix`. For 
`the_matrix_signifs` the significance of a merged cluster will be set to TRUE 
if at least one member of the cluster is TRUE.

```{r}
the_matrix.bak <- the_matrix
the_matrix <- the_matrix.bak

the_matrix_signifs.bak <- the_matrix_signifs
the_matrix_signifs <- the_matrix_signifs.bak
...
the_matrix <- merge_features(the_matrix, clusters, mean)
saveRDS(the_matrix, file = file.path(outdir, 
  "the_matrix_drugsgenesremoved_noNAs_featsmerged_wide.rds"))
# significance is set to yes if any cluster member is significant
the_matrix_signifs <- merge_features(the_matrix_signifs, clusters, any)
saveRDS(the_matrix_signifs, file = file.path(outdir, 
  "the_matrix_signifs_drugsgenesremoved_noNAs_featsmerged_wide.rds"))
```
